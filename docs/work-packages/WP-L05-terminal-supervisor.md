...
- Local migrations `004_execution_supervision.sql`, `005_launch_cleanup.sql`, `006_execution_observations.sql` and `007_native_inspection.sql`; bounded local recovery, observation and command records, with atomic unstarted cleanup exclusion, durable process-observation checkpoints and retained daemon-observed descendant history. No new cloud business state outside C09 is introduced.
