...
| --- | --- | --- | --- | --- |
| AG-01 | Three humans have different workspace/project/runner access and cannot cross tenant boundaries | C04 | C02, C06, C07, C08, L08, W01 | Exhaustive permission matrix at API/MCP/repository/UI layers; ungranted Mac wake rejected |
...
| AG-08 | Revocation disables runner/CLI/OAuth/membership/project authority before cleanup and closes live channels | C04 | C05, C06, C07, C09, L08, A01, E02, X03 | Race tests at command claim, final auth, API/MCP call, token renewal, pending replay, and socket close |
...
| Gate | Release claim | Primary owner | Contributing packages | Required proof |
| --- | --- | --- | --- | --- |
| SG-01 | Browser identity, passkey step-up, CLI credentials, runner credentials, and remote MCP credentials cannot substitute for one another | G01 | C02, C03, C05, C06, X03 | Credential-type confusion matrix and route-level rejection tests |
...
| SG-04 | Auth/passkey/OAuth plugin defaults cannot expose bypass routes or flows | G01 | C02, C03, X03 | Route inventory snapshot and negative tests for Organization, passkey mutation, DCR/client CRUD/client credentials/JWT |
...
| IC-1 Team workspace | W01 | IC-0 plus human auth, step-up, tenant/project permission and work-record tests |
