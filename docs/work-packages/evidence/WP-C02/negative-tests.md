...
- Alias cookies, malformed cookie encoding, expired sessions, and session cookies presented to MCP, OAuth token, runner, webhook, or artifact routes do not authenticate those surfaces. MCP, OAuth token, and webhook probes return 401 `credential_confusion`; the runner probe returns the owning C06 uniform 403 `request_rejected`.
