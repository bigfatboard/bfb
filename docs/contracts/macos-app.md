...
The exact clean-checkout gate passed at `0cc3cb9`, including three available-session relaunch cycles with consent-denied outcomes. Native visual review then prompted three copy/accessibility corrections, committed as `c1653e6`. Full repository verification and cross-build pass at that revision. An explicit Terminal-consent request did not produce an observed decision, and subsequent native delivery timed out with Apple Event error `-1712`; bounded tracing confirmed Terminal opened before the send timed out. The app reported `app_delivery_unknown`, not a success or automatic retry. The underlying Terminal/OS dialog state remains unresolved, and the updated exact gate is not certified. A console that locks during certification does not count as available-session proof.
