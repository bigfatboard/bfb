...
Development provisioning was completed with Timo's approval on 12 September. The explicit BFB App ID and Mac development profile authorize Associated Domains for the existing developer certificate and test Mac. The build accepts Apple's scalar wildcard profile entitlement as well as an array allowlist; the app's signed entitlement still contains only explicitly configured hosts. Verified helper paths are standardized before Terminal command construction, including macOS's `/private/var` alias.

The exact clean-checkout gate is certified at `c1653e6`, together with repository verification, Linux cross-build and the clean-worktree assertion. The final native run proves the fixed-helper Terminal invocation and all three available-session relaunch cycles; notifications correctly report denied access. An earlier clean run at `0cc3cb9` also completed three cycles with Terminal consent denied. Native visual review produced the copy/accessibility corrections included in the certified revision.

An intermediate explicit consent request did not produce an observed decision, and a subsequent Apple Event timed out with `-1712`. That run failed and reported `app_delivery_unknown`; it was not counted as success or automatically retried as a business action. After Timo reported allowing access and starting Terminal, the unchanged certified code passed the exact gate. The precise system-dialog state was not independently observed. The [bounded evidence](../work-packages/evidence/WP-L04/manifest.json) records these distinctions and release-environment limitations. A console that locks during certification does not count as available-session proof.
