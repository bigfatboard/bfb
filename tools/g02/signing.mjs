...
try {
  ...
  assert.ok(existsSync(join(signed.app, "Contents/Helpers/bfb")), "signed bundle embeds the hook launcher");
  assert.ok(existsSync(join(signed.app, "Contents/embedded.provisionprofile")), "signed bundle embeds the profile");
} finally {
...
await writeFile(evidence, await format(JSON.stringify(result, null, 2), { ...options, parser: "json" }));
