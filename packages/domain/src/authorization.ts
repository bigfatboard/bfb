...
export function loadPrincipal(
  ...
  if (member.role === "restricted_member") {
    ...
    projectIds = (
      db.prepare(`SELECT id FROM projects WHERE workspace_id = ?`).all(workspaceId) as Array<{
        id: string;
      }>
    ).map((row) => row.id);
  }
...
export function bumpMemberEpoch(db: SqlDatabase, workspaceId: string, humanId: string): number {
  const current = db
    .prepare(
