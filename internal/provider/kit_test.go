...
func TestProviderOwnedVersionParsers(t *testing.T) {
	...
	for _, descriptor := range providers.Descriptors() {
		...
		// manifest. P01 certifies codex; every other real descriptor stays
		// discovery-only until its owning package amends this set.
		certified := descriptor.Name == "codex"
		...
		if certified && (len(descriptor.Manifest.Capabilities) == 0 || len(descriptor.Manifest.TestedVersions) == 0 || descriptor.Adapter == nil) {
			t.Fatal("certified provider lacks its verified manifest")
		}
	}
}
