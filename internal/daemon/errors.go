...
var failures = map[string]struct {
	...
	"execution_assignment_invalid":  {"authorization_denied", "The local execution does not match its immutable command and assignment.", 3},
	"execution_intent_consumed":     {"conflict", "This one-time Terminal intent cannot be claimed by another process or used to start again.", 6},
	"execution_capacity":            {"unavailable", "Local execution storage is full; preserve pending recovery and inspect local state.", 4},
